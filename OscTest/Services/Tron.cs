...
namespace OscVisualizer.Services
{
    internal class Tron : IAudioVisualizer
    {
        ...
        public List<XYPoint> ProcessAudio(WasapiCapture capture, WaveInEventArgs e)
        {
            ...
            {
                wallScenes[i].Translation = new Vector3(0, (60f * i) + ((float)(time / 5) % 60f), 0f);
            }
            //床をスクロールする
            int floorIndex = 0;
            for (int x = 0; x < 4; x++)
            {
                for (int y = 0; y < 4; y++)
                {
                    floorScenes[floorIndex].Translation = new Vector3((200f * x) - (200f * 2), (200f * y) - (200f * 2) + ((float)(time / 5) % 200f), 0f);
                    floorIndex++;
                }
            }
            //カメラの位置をランダムに切り替える
