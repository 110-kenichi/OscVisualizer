...
{
    internal class MegaDrive : IAudioVisualizer
    {
        ...
        /// </summary>
        /// <remarks>This constructor sets up the visualizer view for the MegaDrive instance. Use this
        /// constructor when you need to create a new MegaDrive with its default visualizer configuration.</remarks>
        public MegaDrive()
        {
            ...
            var bodymesh = MeshBuilder.BuildIndexedMesh(bodymodel, vertexMergeEpsilon: 1e-3f);

            var bodyScene = new SceneMeshInstance(bodymesh);
            bodyScene.RotationZDeg = -90;
            ...
            _renderer = new HiddenLineSilhouetteSceneRenderer
            {
                ...
                // ===== シーン全体変換 =====
                SceneScale = 0.01f,
                SceneTranslation = new Vector3(0, 0, 30.0f),
        ...
        private void UpdateRotation(float kick, float snare, float hat)
        {
            ...
            if (Math.Abs(currentAngleX - targetAngleX) < 0.1f)
                targetAngleX = (float)(random.NextDouble() * 60);
            if (Math.Abs(currentAngleY - targetAngleY) < 0.1f)
                targetAngleY = (float)(random.NextDouble() * 90 - 45);
            if (Math.Abs(currentAngleZ - targetAngleZ) < 0.1f)
                targetAngleZ = (float)(random.NextDouble() * 30 - 15);
            targetAngleX = Math.Max(targetAngleX, Math.Min(kick, 60));
            targetAngleY = targetAngleY / (kick / 2 + 1);
            targetAngleZ = targetAngleZ / (kick / 2 + 1); ;
            // 2. スムーズに補間する (線形補間の例)
        ...
        public List<XYPoint> ProcessAudio(WasapiCapture capture, WaveInEventArgs e)
        {
            ...
            float scale = 15f + kick * 2;

            // レンダリング
            UpdateRotation(kick, snare, hat);
            _renderer.SceneScale = scale;
            float x = -5.0f * (1f - MathF.Exp(-0.25f * kick));
            _renderer.SceneTranslation = new Vector3(x, 0, 30.0f);
