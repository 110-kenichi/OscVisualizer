...
namespace OscVisualizer.Services
{
    ...
    internal class Processing : IAudioVisualizer
    {
        ...
        private sealed class WhileStatement(IExpression condition, List<IStatement> body) : IStatement
        {
            private readonly IExpression _condition = condition;
            private readonly List<IStatement> _body = body;
        ...
        private static class ScriptParser
        {
            ...
            private sealed class Parser(List<Token> tokens)
            {
                ...
                private IStatement ParseStatement()
                {
                    ...
                    if (Current.Kind == TokenKind.Identifier && Current.Text.Equals("while", StringComparison.OrdinalIgnoreCase))
            ...
            private sealed class Tokenizer(string source)
            {
                ...
                private bool ShouldInsertImplicitSemicolon(List<Token> tokens)
                {
                    ...
                    {
                        string nextWord = PeekNextSignificantWord(_index + 1);
                        if (string.Equals(nextWord, "for", StringComparison.OrdinalIgnoreCase))
                            return false;
                    }
                ...
                private string PeekNextSignificantWord(int start)
                {
                    int i = start;
