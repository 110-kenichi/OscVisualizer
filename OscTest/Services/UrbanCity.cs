...
{
    internal class UrbanCity : IAudioVisualizer
    {
        ...
        public UrbanCity()
        {
            ...
            topmodel.NormalizeToUnitCube();
            var topmesh = MeshBuilder.BuildIndexedMesh(topmodel, vertexMergeEpsilon: 5e-4f);
            ...
            _renderer = new HiddenLineSilhouetteSceneRenderer
            {
                ...
                // ===== シーン全体変換 =====
                //SceneScale = 0.2f,
                SceneTranslation = new Vector3(0, 30f, 40.0f),

                SceneRotationXDeg = 0f,
                SceneRotationYDeg = 135f,
                SceneRotationZDeg = 90f,
        ...
        private void UpdateRotation(float kick, float snare, float hat)
        {
            ...
            //_renderer.SceneRotationXDeg = 0;
            //_renderer.SceneRotationYDeg = 135;
            //_renderer.SceneRotationZDeg = 90;

            // 3. この currentAngle を描画時の回転行列に適用する
            // (例: graphics.RotateTransform(currentAngle); )

            if (Math.Abs(currentTopAngleX - targetTopAngleX) < 0.1f &&
                Math.Abs(currentTopAngleY - targetTopAngleY) < 0.1f)
            {
                if (Math.Round(random.NextDouble()) == 0)
                {
                    targetTopAngleX = (float)(random.Next(2) == 0 ? -360 : 360);
                }
                else
                {
                    targetTopAngleY = (float)(random.Next(2) == 0 ? -360 : 360);
                }
            }
            float topLerpSpeed = 0.1f + 0.15f * hat;
            currentTopAngleX += (targetTopAngleX - currentTopAngleX) * topLerpSpeed;
            currentTopAngleY += (targetTopAngleY - currentTopAngleY) * topLerpSpeed;
            topScene.RotationXDeg = 0; // currentTopAngleX;
            topScene.RotationYDeg = 0; // currentTopAngleY;
            topScene.RotationZDeg = currentAngleY + (float)_sw.Elapsed.TotalMilliseconds / 25;
        }
        ...
        public List<XYPoint> ProcessAudio(WasapiCapture capture, WaveInEventArgs e)
        {
            ...
            float scale = 1f + kick / 10f;

            // レンダリング
            UpdateRotation(kick, snare, hat);
            _renderer.SceneScale = 5 * scale * scale;
